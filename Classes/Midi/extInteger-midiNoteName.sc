MidiNoteNames[] {
    ...
    *note{|integerOrString|
        ^if(integerOrString.isKindOf(String) or: { integerOrString.isKindOf(Symbol) }, {
            var key = integerOrString.asSymbol;
            this.items.invert.at(key)
        }, {
            var index = integerOrString.asInteger;
            this.items[index]
        })
    }
}
